...
void desenharTelaComer() {
  if (cliquesComida >= 0 && cliquesComida < 10) {
    ...
    if (comidaSelecionada == 0) { 
      telaVirtual.fillRect((cx) * 5, (cy - raio - 2) * 5, 1 * 5, 3 * 5, TFT_WHITE);
      telaVirtual.fillRect((cx + 1) * 5, (cy - raio - 1) * 5, 2 * 5, 1 * 5, TFT_WHITE);
      telaVirtual.fillRect((cx - 2) * 5, (cy - 2) * 5, 5 * 5, 5 * 5, TFT_WHITE);
    } else { 
      telaVirtual.fillRect((cx - 1) * 5, (cy - 1) * 5, 3 * 5, 3 * 5, TFT_WHITE);
      telaVirtual.drawRect((cx - 3) * 5, (cy - 3) * 5, 7 * 5, 7 * 5, TFT_WHITE);
      telaVirtual.fillRect((cx - 5) * 5, (cy) * 5, 2 * 5, 1 * 5, TFT_WHITE);
      telaVirtual.fillRect((cx + 4) * 5, (cy) * 5, 2 * 5, 1 * 5, TFT_WHITE);
    }
...
end_str = "void updateTelaVirtual() {"
...
ino = ino.replace("obsX -= 4;", "obsX -= 1;") # Logical speed
ino = ino.replace("obsX -= 2;", "obsX -= 1;") 
ino = ino.replace("pularY = -25;", "pularY = -5;") # Logical jump height
...
# Bypass Brincar Menu
# Change case 1 in left button (Menu Cursor)
menu_logic = """        case 1:
          ...
          hitObs = false;
          break;"""
ino = re.sub(r"case 1:\s*subMenuCursor = 0;\s*estadoAtual = TELA_SUBMENU_BRINCAR;\s*break;", menu_logic, ino)

# In left button click
ino = re.sub(r"case TELA_SUBMENU_BRINCAR:\s*if \(estadoAtual == TELA_SUBMENU_COMER\) subMenuCursor = \(subMenuCursor \+ 1\) % 2;.*?\n\s*break;", "", ino)
ino = re.sub(r"case TELA_SUBMENU_BRINCAR:\s*executarSubMenuBrincar\(\);\s*break;", "", ino)
