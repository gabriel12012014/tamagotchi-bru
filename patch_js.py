...
# 1. Update primitive drawing functions
code = code.replace("function fillRect(x, y, w, h, c) { ctx.fillStyle = c; ctx.fillRect(x, y, w, h); }",
                    "function fillRect(x, y, w, h, c) { ctx.fillStyle = c; ctx.fillRect(x*5, y*5, w*5, h*5); }")
code = code.replace("function drawRect(x, y, w, h, c) { ctx.strokeStyle = c; ctx.lineWidth = 1; ctx.strokeRect(x+0.5, y+0.5, w-1, h-1); }",
                    "function drawRect(x, y, w, h, c) { ctx.strokeStyle = c; ctx.lineWidth = 1; ctx.strokeRect(x*5+0.5, y*5+0.5, w*5-1, h*5-1); }")

code = code.replace("""    function drawText(txt, x, y, c, size) {
      ctx.fillStyle = c;
...
# TELA_PET
code = re.sub(r"function desenharTelaPet\(now\).*?\}", """function desenharTelaPet(now) {
...
# TELA_MENU
code = re.sub(r"function desenharMenu\(\).*?\}", """function desenharMenu() {
...
# TELA_STATUS
code = re.sub(r"function desenharTelaStatus\(\).*?\}\s*// =+.*?DESENHO DE CORAÇÃO", """function desenharTelaStatus() {
    ...
    // =============================================
    // DESENHO DE CORAÇÃO""", code, flags=re.DOTALL)

# DESENHAR CORACAO
code = re.sub(r"function desenharCoracao\(cx, cy\).*?\}", """function desenharCoracao(cx, cy) {
...
# TELA_CARINHO
code = re.sub(r"function desenharTelaCarinho\(now\).*?\}\s*// =+", """function desenharTelaCarinho(now) {
      const respY = Math.round(Math.sin(now / 800) * 1);
      const pet = getPetSprite();
      const x = Math.floor((27 - pet.w) / 2);
      const y = Math.floor((48 - pet.h) / 2) + respY;
...
# TELA_COMER
code = re.sub(r"function desenharTelaComer\(\).*?\}\s*// =+.*?TELA DOS SUBMENUS", """function desenharTelaComer() {
    ...
    // =============================================
    // TELA DOS SUBMENUS""", code, flags=re.DOTALL)

# SUBMENU
code = re.sub(r"function desenharSubMenu\(titulo, opcoes, total\).*?\}\s*// =+", """function desenharSubMenu(titulo, opcoes, total) {
...
# TELA PULAR
code = re.sub(r"function desenharTelaPular\(now\).*?\}\s*// =+", """function desenharTelaPular(now) {
...
# MENSAGEM SUCESSO
code = re.sub(r"function desenharMensagemSucesso\(\).*?\}\s*// =+", """function desenharMensagemSucesso() {
...
# GAME OVER
code = re.sub(r"function desenharGameOver\(\).*?\}\s*// =+", """function desenharGameOver() {
...
# render logic jumping - adjust pularY and obsX
code = code.replace("obsX -= 8;", "obsX -= 2;") # Slower physical obs speed
code = code.replace("pularY = -25;", "pularY = -5;")
code = code.replace("if (obsX > 40 && obsX < 75 && pularY === 0) {", "if (obsX > 8 && obsX < 15 && pularY === 0) {")
code = code.replace("if (obsX < -20 && !hitObs) {", "if (obsX < -4 && !hitObs) {")
code = code.replace("obsX = 135;", "obsX = 27;")

# Reset pular in executarSubMenuBrincar
code = code.replace("obsX = 135;", "obsX = 27;")
